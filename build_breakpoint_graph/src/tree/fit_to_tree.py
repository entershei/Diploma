...
def fit_to_tree_graphviz(
    ...
    def rec_convert_t_to_graphviz(n, g, edge_to_w, acc_name):
        g.add_node(
            acc_name,
            label=(n.name if n.is_leaf() else ""),
            shape=("ellipse" if n.is_leaf() else "point"),
        )
...
def count_tree_errors(
    tree_file,
    infercars_file,
    dh,
    alignment_type,
    filter_alt=True,
    error_func=lambda y, y_tree: np.sqrt(np.sum(np.power((y - y_tree) / y, 2))),
    unroot=True,
):
    distances, sp_pairs = __count_species_distances(infercars_file, filter_alt)
    for sp1, sp2 in sp_pairs:
...
def count_alignment_pairs_alignment(infercars_file, distance_holder, filter_alt=True):
    ...
    est_ds = {
        est: [distances[(sp1, sp2, est)] for sp1, sp2 in sp_pairs] for est in etimators
    }
